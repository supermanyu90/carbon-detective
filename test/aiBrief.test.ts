...
describe("prompt + fallback", () => {
  ...
  it("user message is deterministic and lists findings", () => {
    const req = buildAnalystRequest("class", "MINOR LEAKS", totals, [findingFor("c_corridor")]);
  ...
  it("local fallback handles both empty and populated audits", () => {
    const empty = buildAnalystRequest("home", "SPOTLESS", { co2: 0, cost: 0, kwh: 0, water: 0, fuel: 0 }, []);
