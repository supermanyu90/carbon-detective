...
export function buildAnalystRequest(
  ...
  return {
    ...
    findings: found.slice(0, MAX_FINDINGS).map((f) => ({
      ...
      zone: f.clue.zone,
      title: f.clue.q,
      severity: severityOf(f.impact.co2, f.clue.water),
      co2: Math.round(f.impact.co2),
      cost: Math.round(f.impact.cost),
      count: f.count,
    })),
  };
}
...
export function localFallbackBrief(req: AnalystRequest): string {
  ...
  const lead = top
    .map(
      (f, i) => `${i + 1}. ${shorten(f.title)} — about ${f.co2} kg CO₂ and ₹${f.cost} a year`,
    )
    .join("; ");
  return (
