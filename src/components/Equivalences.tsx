...
export function Equivalences({ co2, water, reduceMotion }: Props) {
  const trees = co2 / 22; // ~22 kg CO₂ absorbed per mature tree per year
  const km = co2 / 0.18; // ~180 g CO₂ per car-km
  const phones = co2 / 0.01; // ~10 g CO₂ per smartphone charge
  const buckets = water / 15;
    ...
    <div className="equiv equiv-board">
      ...
      <div className="equiv-grid">
        <div className="equiv-item">
          <span className="equiv-ico grow" aria-hidden="true">🌳</span>
          <span className="equiv-num">
            <CountUp value={Math.max(1, trees)} reduceMotion={reduceMotion} />
          </span>
          <span className="equiv-lbl">mature tree{trees >= 1.5 ? "s" : ""} working a full year</span>
        </div>
        <div className="equiv-item">
          <span className="equiv-ico drive" aria-hidden="true">🚗</span>
          <span className="equiv-num">
        ...
        <div className="equiv-item">
          <span className="equiv-ico charge" aria-hidden="true">🔋</span>
          <span className="equiv-num">
        ...
        {water > 0 && (
          <div className="equiv-item">
            <span className="equiv-ico pour" aria-hidden="true">🪣</span>
            <span className="equiv-num">
